...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Jaime Sáez de Buruaga Brouns");
MODULE_DESCRIPTION("UCM/FDI/LIN/Prac3/ParteA");
...
#define BUFFER_LENGTH NR_LEDS*(NR_BYTES_BLINK_MSG + 3) + 1
...
{
	struct usb_blink *dev=file->private_data;	// Estructura USB
	char kbuf[BUFFER_LENGTH];					// Buffer para pasar a modo kernel
	char aux[NR_BYTES_BLINK_MSG];				// Buffer para almacenar el patron a buscar en SSCANF
	char *buffer;								// Buffer para almacenar el TOKEN
	char *found;								// Búffer para almacenar el SUBTOKEN
	unsigned int color;							// Entero sin signo para el COLOR
	int available_space = BUFFER_LENGTH - 1;	// Nº de leds máximo
	int num_led;								// Índice de leds
	int i = 0;									// Índice
	int retval = 0;								// Valor de retorno en caso de fallo
	char *message = NULL;						// Cadena a imprimir en los leds
	...
	*/
	if(len > available_space) {
		printk(KERN_INFO "blinkstick: not enough space");
	...
	kbuf[len] = '\0';
	*off+=len;
		/*Reserva memoria DMA para mensaje*/
	message = kmalloc(NR_BYTES_BLINK_MSG,GFP_DMA);

	buffer = (char*)vmalloc(BUFFER_LENGTH);
	strcpy(buffer, kbuf);
		/*Entra si encuentra algo en kbuf*/
	if(strlen(buffer) > 1){
		int count = 0;
		while((found = strsep(&buffer, ",")) != NULL){
				/* Almaceno en NUM_LED el led a cambiar */
			if(sscanf(found, "%i", &num_led) == 1){
				count++;
				if(num_led > 7 || num_led < 0){
					printk(KERN_INFO "Error, led %i value is out of scope [0,7]", num_led);
					retval = -EINVAL;
					goto error;
				}	
					/* Copio en AUX (nrLed:COLOR)*/
				strncpy(aux, found, 2);	
					/* Continuo el patron para AUX*/
				aux[2] = '%'; aux[3] = 'x'; aux[4] = '\0'; /* Guardo en AUX el patron para sscanf*/ 
					/* Busco en FOUND el color */
				if(sscanf(found, aux, &color) == 1){	/*Ya tengo LED y COLOR*/
					sample_colors[num_led] = color;
					printk(KERN_INFO "In data received, num_led: %i, color: %i\n", num_led, color);
				}
			}
		}
		if(count == 0){
			...
			/* La cadena escrita está vacía -> apagar los leds*/
		for(i = 0; i < NR_LEDS; i++)
			sample_colors[i] = 0x000000;
		printk(KERN_INFO "Leds turned off\n");	/* Aquí accede correctamente */
	}
	/* zero fill */
	...
	for(i = 0; i < NR_LEDS; i++){
			/* Me posiciono en led[i]*/
		message[2] = i;
			/* Escogo el color de la tabla creada anteriormente */
		color = sample_colors[i];
			/* Introduzco color en el mensaje */
